...
> **นี่คือ stack template** — copy ไฟล์นี้เป็น `STACK_CONTEXT.md` ของ project แล้ว customize
> ดูวิธีใช้ที่ [README.md](README.md)

---

## Purpose

Template สำหรับ project ที่ใช้:
...
SA copy template นี้แล้ว fill in ค่าที่ขาด และลบ section ที่ project นี้ไม่ใช้
...
> ลบ section นี้ถ้า project ไม่มี SSR/SSG frontend
...
- Route: App Router — ห้ามใช้ Pages Router
- Environment variable: ใช้ `NEXT_PUBLIC_` prefix เฉพาะตัวที่ expose ถึง browser
- shadcn/ui: ใช้ `pnpm dlx shadcn@latest add <component>` — component อยู่ที่ `components/ui/`

### Constraints

> SA และ Dev ต้องอ่าน release notes ของ Next.js version ที่ใช้ก่อนเขียน code ทุกครั้ง (breaking changes ต่อ major version)

---

## 3. Frontend SPA — Vite + React

> ลบ section นี้ถ้า project ไม่มี SPA (admin/internal tool)
...
- shadcn/ui: ใช้ `pnpm dlx shadcn@latest add <component>` — component อยู่ที่ `src/components/ui/`
...
| ใช้ plain map แทนได้เมื่อ | ข้อมูลไม่ต้องการ eviction และ load ครั้งเดียวตอน startup |
...
- ห้าม run raw query ใน `usecase/` หรือ `service/`

> **SA สามารถเลือก storage type อื่นได้ตาม use case** — ต้องระบุใน ADR
> ตัวเลือก: MongoDB (document), DynamoDB (key-value), S3-compatible (object store)
...
- IdP: **[SA ระบุ — Keycloak / Azure AD / Okta / อื่น]** — ระบุใน ADR
...
- Access token ต้อง validate ผ่าน JWKS endpoint
...
> Framework instrumentation — SA เลือกตาม router และ driver (default: `otelfiber`)
...
> ทางเลือก: `@vercel/otel` แทน manual setup
...
**Constraints:**
- Export protocol: OTLP — ห้าม tie ถึง vendor-specific format โดยตรง
...
- Personal data: ชื่อ, เลขบัตร, เบอร์โทร, email, ที่อยู่, ข้อมูลสุขภาพ
- ทุก feature ที่เก็บ personal data ต้องระบุ retention period ใน Solution Doc
- ต้องมี deletion mechanism ตาม retention period
...
| Field | Default | ทางเลือก |
|-------|---------|---------|
| HTTP Router | Fiber v3 | Chi / Echo / อื่น + เหตุผล |
| Validation Library | go-playground/validator/v10 | อื่น |
| Config Library | caarlos0/env | koanf (multi-source) / อื่น |
| Auth Method | OAuth2 + OIDC + PKCE | — IdP: SA เลือกต่อ project |
| SPA State Management | Zustand | TanStack Query / อื่น |
| SPA Router | TanStack Router | React Router v7 / อื่น |
| Migration Tool | golang-migrate | goose / อื่น |
